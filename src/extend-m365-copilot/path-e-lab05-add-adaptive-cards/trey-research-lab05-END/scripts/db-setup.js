...
(async () => {
  const connectionStringOrAccountName = process.argv[2]
    ? process.argv[2]
    : "UseDevelopmentStorage=true";
  const reset =
    process.argv[3] === "--reset" || process.argv[3] === "-r" ? true : false;

  // Determine if we're using a connection string or managed identity
  const isConnectionString =
    connectionStringOrAccountName.includes("=") ||
    connectionStringOrAccountName === "UseDevelopmentStorage=true";
  ...
  if (isConnectionString) {
    ...
    const url = `https://${connectionStringOrAccountName}.table.core.windows.net`;
    tableServiceClient = new TableServiceClient(url, credential);
    getTableClient = (tableName) => new TableClient(url, tableName, credential);
  }
  ...
  TABLE_NAMES.forEach(async (tableName, index) => {
    ...
    const jsonString = fs.readFileSync(
      path.resolve(__dirname, "db", `${tableName}.json`),
      "utf8"
    );

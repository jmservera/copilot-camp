...
export default class DbService<DbEntityType> {
  private storageAccountName = process.env.STORAGE_ACCOUNT_NAME;
  private storageAccountConnectionString =
  ...
  constructor(okToCacheLocally: boolean) {
    if (!this.storageAccountName && !this.storageAccountConnectionString) {
      throw new Error(
        "STORAGE_ACCOUNT_NAME or STORAGE_ACCOUNT_CONNECTION_STRING must be set"
      );
    }
  ...
  private getTableClient(tableName: string): TableClient {
    if (this.storageAccountConnectionString) {
      ...
      const url = `https://${this.storageAccountName}.table.core.windows.net`;
      return new TableClient(url, tableName, credential);
    }
  }

  async getEntityByRowKey(
    tableName: string,
    rowKey: string
  ): Promise<DbEntity> {
    if (!this.okToCacheLocally) {
      const tableClient = this.getTableClient(tableName);
      const result = this.expandPropertyValues(
        (await tableClient.getEntity(tableName, rowKey)) as DbEntityType
      );
  ...
  async getEntities(tableName: string): Promise<DbEntity[]> {
    if (!this.okToCacheLocally || this.entityCache.length === 0) {
      ...
      for await (const entity of entities) {
        ...
        if (
          this.entityCache.find((e) => e.rowKey === entity.rowKey) === undefined
        ) {
